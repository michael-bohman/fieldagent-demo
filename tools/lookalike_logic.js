...
function mount(root, DATA, opts = {}) {
  ...
  const blockedMsg = what => `${what} is part of the full FieldAgent account. This sample is view-only.`;
  ...
  $('ruler').addEventListener('click', () => { measuring = !measuring; measurePts = []; $('ruler').classList.toggle('on', measuring); const m = $('measure'); m.hidden = !measuring; m.textContent = isTouch ? 'Tap two points on the map to measure.' : 'Click two points on the map to measure.'; canvas.style.cursor = measuring ? 'crosshair' : ''; dirty = true; });
  ...
  // ---------- navigation ----------
  function defaultLayers(fid) {
    const f = FIELD[fid]; const out = []; const s = f.surveys.find(x => hasImg(`${fid}_${x.key}_ndvi`)) || f.surveys[0]; if (!s) return out;
  ...
  function goto(view) { state.view = view; state.menu = null; render(); }

  // ---------- tips (demo annotations; dismissible, remembered per browser) ----------
  function tip(id, html) { if (state.tipsDone.has(id)) return ''; return `<div class="tip-card" data-tip="${id}">${icon('i-info')}<div>${html}</div><button type="button" data-act="tipclose" data-tip="${id}" aria-label="Dismiss tip">${icon('i-close', 'ico xs')}</button></div>`; }
  ...
  function renderUpload() {
    ...
    const photos = `<section class="card"><div class="card-title">Flight Details</div>
        <div class="field-label" style="margin:4px 0 0 6px">Altitude above ground</div>
      ...
      <section class="card dropzone" data-act="drop" role="button" tabindex="0">${icon('i-upload')}<b>Add JPEG or TIFF photos</b><span>Drop files here to upload.</span><span class="na">Up to 2,000 photos per flight on Scout · calibration-panel photos in the set calibrate the stitch</span></section>`;
  ...
  function outsideHandler(e) {
    ...
    else if (act === 'download') { openLead(t.dataset.what, t); }
    else if (act === 'pband') { p.band = t.dataset.band; p.zoom = 1; renderPhoto(); emit('photo_band_changed', { band: p.band }); }
    else if (act === 'pmeta') { p.meta = !p.meta; renderPhoto(); }
    else if (act === 'psub') { p.sub[t.dataset.sub] = !p.sub[t.dataset.sub]; renderPhoto(); }
    else if (act === 'pzoom') { p.zoom = clamp(p.zoom * (t.dataset.dir === '1' ? 1.4 : 1 / 1.4), 1, 4); const im = modalEl.querySelector('.photo-stage img'); if (im) im.style.transform = `scale(${p.zoom})`; }
    else if (act === 'pnav') { const L = findLayer(p.uid); const n = photoSamples(L).length; p.idx = clamp(p.idx + (+t.dataset.dir), 0, n - 1); const smp = photoSamples(L)[p.idx]; if (!(smp.img && smp.img[p.band])) p.band = firstBand(L, smp); p.zoom = 1; renderPhoto(); dirty = true; }
    else if (act === 'rtitle-edit') { if (r.editTitle) { const inp = reportEl.querySelector('[data-act="rtitle"]'); if (inp && inp.value.trim()) r.title = inp.value.trim(); } r.editTitle = !r.editTitle; render(true); }
  ...
  panel.addEventListener('click', e => {
    ...
    else if (act === 'zonedone') { goto('field'); flyTo(fitField(), 600); }
    else if (act === 'showmore') { state.showMore = !state.showMore; render(true); }
    else if (act === 'src') { state.sources[t.dataset.src] = !state.sources[t.dataset.src]; render(true); if (t.dataset.src === 'satellite' && state.sources.satellite) emit('satellite_opened'); }
    ...
    else if (act === 'toggle') { e.stopPropagation(); const l = findLayer(+t.dataset.uid); l.visible = !l.visible; render(true); }
    else if (act === 'remove') { e.stopPropagation(); state.layersByField[state.fid] = layers().filter(x => x.uid !== +t.dataset.uid); render(true); }
    else if (act === 'menu') { state.menu = state.menu === t.dataset.menu ? null : t.dataset.menu; render(true); }
    else if (act === 'viz') { setViz(L, t.dataset.viz); state.menu = null; ensure(L); render(true); emit('visualization_changed', { viz: t.dataset.viz }); }
    else if (act === 'scale') { L.col.scale = t.dataset.scale; state.menu = null; render(true); }
    else if (act === 'mode') { L.col.mode = t.dataset.mode; render(true); }
    else if (act === 'clip') { L.clipped = t.dataset.clip === '1'; render(true); }
    else if (act === 'includeout') { L.col.includeOut = !L.col.includeOut; render(true); }
    else if (act === 'bintable') { state.binTable = !state.binTable; render(true); }
    else if (act === 'openphoto') { openPhoto(+t.dataset.uid, +t.dataset.idx); }
    else if (act === 'sort') { state.sort = t.dataset.sort; state.menu = null; render(true); }
    else if (act === 'season') { state.menu = null; render(true); }
    else if (act === 'osurvey') { state.order.survey = t.dataset.survey; state.order.picks = {}; state.order.aligned = {}; state.menu = null; render(true); }
    else if (act === 'opick') { const k = t.dataset.key; state.order.picks[k] = !state.order.picks[k]; if (!state.order.picks[k]) delete state.order.aligned[k]; render(true); }
    else if (act === 'oalign') { const k = t.dataset.key; state.order.aligned[k] = !state.order.aligned[k]; render(true); }
    ...
    else if (act === 'utype') { state.upload.type = { 'Fully Stitched Mosaic': 'mosaic', 'QuickTile': 'quicktile', 'Individual Photos': 'photos' }[t.dataset.val]; render(true); }
    else if (act === 'ukind') { state.upload.kind = t.dataset.val; render(true); }
    else if (act === 'horizon') { state.upload.horizon = !state.upload.horizon; render(true); }
    else if (act === 'drop' || act === 'usubmit') { toast('Uploads happen in your own FieldAgent account. This sample is view-only.'); }
    else if (act === 'rsec') { const k = t.dataset.sec; state.report.secOpen[k] = !state.report.secOpen[k]; render(true); }
    else if (act === 'rtoggle') { const k = t.dataset.key; state.report[k] = !state.report[k]; render(true); }
    else if (act === 'paper') { state.report.paper = t.dataset.paper; state.menu = null; render(true); }
    else if (act === 'download') { const hadMenu = !!state.menu; openLead(t.dataset.what, t); if (hadMenu) render(true); }
  });
  ...
  panel.addEventListener('input', e => {
    const t = e.target.closest('[data-act]'); if (!t) return; const act = t.dataset.act;
    if (act === 'search') { state.search = t.value; const scroll = panel.querySelector('.panel-scroll').scrollTop; render(); const inp = panel.querySelector('[data-act="search"]'); inp.focus(); inp.setSelectionRange(inp.value.length, inp.value.length); panel.querySelector('.panel-scroll').scrollTop = scroll; return; }
    if (act === 'alt') { state.upload.alt = Math.round(parseFloat(t.value)); $('alt-caption').textContent = `${state.upload.alt} ft`; return; }
    ...
    else if (act === 'bins') { L.col.bins = Math.round(v); patchColorization(L); }
    else if (act === 'rmin') { const [lo, hi] = vizRange(L); const gap = (hi - lo) / 100; L.col.min = Math.min(v, L.col.max - gap); t.value = L.col.min; patchColorization(L); }
    else if (act === 'rmax') { const [lo, hi] = vizRange(L); const gap = (hi - lo) / 100; L.col.max = Math.max(v, L.col.min + gap); t.value = L.col.max; patchColorization(L); }
  });
  ...
  Promise.all(first.map(loadImage)).then(() => {
  ...
  return { state, openField, openFields, flyTo, openPhoto, openLead };
}
return { mount };
})();
